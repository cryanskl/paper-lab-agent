...
低温等离子体文献检索与理解系统：确定性检索层（按白名单 ISSN+关键词抓元数据）+ 大模型理解层（翻译 / RAG / 化学库抽取）。详见 `PRD_等离子体文献系统.md`。

## 真理来源（不得擅自改动）

- **数据模型** = `schema.sql`。不得自行新增表、改字段名或类型。确需变更：先改 `schema.sql` 并说明，再写代码。
- **接口契约** = `接口设计文档.md`。端点路径、方法、请求/响应结构以它为准，不得自创端点或改路径。
- **范围与优先级** = PRD。不做 PRD 第 2 节"非目标"里的事。
- **任务与顺序** = `任务拆分_开发路线.md`。一次只做一个任务，满足其验收标准才算完成。
...
每个任务交付时：(1) 满足 `任务拆分_开发路线.md` 中该任务的全部验收标准；(2) 本地能 `uvicorn` 起服务并验证（给出 curl 或最小测试）；(3) 不破坏已通过的既有任务。
