...
本系统的核心策略是把“发现文献”和“理解文献”分层：发现层用确定性的学术元数据 API、期刊白名单、ISSN、用户搜索词和日期范围保证来源可控；联网命中沉淀到本地并缓存搜索记录。理解层在用户合法获取或上传 PDF 后，再用 GROBID、翻译、RAG 和抽取能力帮助理解内容。
...
- **G2 确定性发现链路**：使用 ISSN、用户指定的 OR/AND 搜索词、日期窗口、结果上限和 DOI 去重构建可追溯搜索流程，不让大模型决定文献是否存在或是否被发现。
...
| 系统维护者 | 维护期刊白名单、搜索任务和分类体系 | 搜索可追溯，配置可调整，异常可定位 |

### 3.2 典型场景

1. 仿真工程师希望找 “Ar/O2 ICP chemistry” 相关论文。
2. 工程师选择白名单期刊和年份，输入一个或多个搜索词并选择 OR/AND 与最多返回数量。
3. 系统从 OpenAlex / Crossref 拉取相关元数据，按 ISSN、日期和搜索逻辑复核，用 DOI 去重入库，并通过 Unpaywall 补合法 OA 链接。
4. 工程师直接查看按相关度返回的有限结果；重复搜索优先复用 24 小时本地缓存。
...
确定性检索层负责发现和维护文献元数据。输入为期刊白名单、ISSN、用户搜索词、OR/AND、日期窗口和结果上限；处理流程为学术 API 相关度查询、本地一致性复核、DOI 去重、OA 链接补全、搜索批次预览和缓存记录。手动联网搜索新增论文先标记为 `preview`，用户确认后才进入正式本地库；用户可撤销整次搜索，已有 PDF、翻译、索引或化学数据的论文必须保留。输出为 `journals`、`papers`、`crawl_jobs`、`search_history`、`search_results`、`categories`、`paper_categories` 等本地数据。
...
建议技术栈与接口文档保持一致：FastAPI、SQLite、APScheduler、GROBID、Chroma 或 FAISS；唯一前端为 FastAPI 托管的原生 HTML/CSS/JS 工作台。所有默认测试必须可离线、确定性运行；真实网络和真实模型检查必须作为单独集成测试。
...
| P1-J2 | 支持新增期刊，字段包括名称、出版社、平台、URL、print/electronic ISSN、起始年份 | P0 | `POST /journals` | 新增后可在列表与详情中查询 |
| P1-J3 | 支持更新来源信息、年份范围和启停状态 | P0 | `PUT /journals/{id}` | 更新后后续搜索使用新配置 |
...
- `keywords` 仅兼容历史数据库，不再作为期刊级准入门槛；搜索词由每次用户查询提供。
- `year_from` 默认 1990，`year_to=NULL` 表示持续至今。

### 5.2 学术元数据抓取

**目标**：按期刊 ISSN、日期窗口和用户 OR/AND 搜索词从学术元数据 API 获取最相关文献，并写入本地 `papers`。
...
| P1-C6 | 支持搜索词 OR/AND、1–100 全局结果上限、预览后保存/撤销与 24 小时精确查询缓存；期刊旧关键词不参与过滤 | P0 | `search_history`、`search_results`；`POST /crawl/run` | 搜索逻辑正确，跨期刊结果总数不超过上限；新增结果不确认不进入正式本地库；整批可安全撤销，且不删除已有下游数据的论文；重复查询不联网 |
...
| 编号 | 需求 | 优先级 | 数据/API 对齐 | 验收标准 |
| --- | --- | --- | --- | --- |
| P1-P1 | 支持标题、摘要、作者与 DOI 全文检索，使用 SQLite FTS5，并支持 OR/AND 与全局结果上限 | P0 | `papers_fts`；`GET /papers?q=` | 输入关键词返回相关论文且不超过上限 |
...
| 编号 | 需求 | 优先级 | 验收标准 |
| --- | --- | --- | --- |
| P1-U1 | 检索页支持逗号分隔搜索词、OR/AND、20/50/100 返回数量、期刊与日期筛选 | P0 | 用户能组合条件联网检索并看到缓存命中提示 |
...
| P4-U5 | 文献库上传可自动串联解析、翻译、RAG 索引和化学库抽取 | P2 | 上传后无需逐个点击；各步骤状态可见，化学结果仍须人工复核 |
...
| 数据域 | 表 | 产品含义 | 关键约束 |
| --- | --- | --- | --- |
| 期刊白名单 | `journals` | 决定确定性搜索来源 | active 控制启停；ISSN 控制期刊定位；keywords 仅兼容旧数据 |
| 搜索历史 | `search_history`、`search_results` | 判断重复查询能否复用并支持批次确认/撤销 | 完整条件生成 cache_key；成功且未撤销的记录 24 小时内复用；逐篇记录是否为本次新增 |
...
V1 前端以可演示和可操作为目标，不追求复杂管理后台。原生工作台统一承载检索、上传、解析查看、翻译查看、问答、反应集复核和导出，不保留第二套前端。前端不得引入未在 PRD 中定义的账号、云同步、团队协作或自动推荐功能。
...
| ISSN 或搜索表达式配置错误 | 搜索范围偏差 | 上线前核对种子期刊；界面明确展示 OR/AND、年份和结果上限 |
