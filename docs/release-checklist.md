...
The combined release artifact command writes `openapi.json`, `demo-summary.json`, and `release-manifest.json` into the target directory. The manifest records source git commit/branch, source dirty state, demo export audit counts, and file checksums. Validate the directory before handoff so missing files, unreadable artifact paths, unexpected extra files, checksum mismatches, edited summaries, manifest/summary audit count drift, version drift, malformed manifests, missing OpenAPI `system` tag metadata, or missing `ErrorResponse` schema fail before sharing. Package the validated directory when you need a single handoff file; the package report includes `artifact_names`, demo readiness, export formats, `demo_export_audit_entry_counts`, `reaction_set_verified_by`, and `reaction_set_verified_at`. Keep the zip output outside the artifact directory so packaging cannot overwrite or pollute the handoff files. Then validate the zip so the package can be unpacked and rechecked before delivery; the package validation report exposes the same demo evidence. For final handoff after committing, add `--require-clean-source` to validation or packaging to fail if the manifest was exported from a dirty worktree. The output is generated under `out/`, which is ignored by Git. Do not hand-edit the exported schema or summary; regenerate them from the app.
