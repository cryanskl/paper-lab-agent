...
`python scripts/health_check.py --summary-only --compact` 会输出短摘要，包含 `api_status`、`demo_data_ready`、`failed_workflows`、`config_warning_count`、`config_warning_codes` 和 `storage_writable`，适合发布或演示前快速确认 live 环境；搭配 `--check-frontend` 时还会返回 `frontend_ok`、`frontend_status_code` 和 `frontend_url`，搭配 `--check-external` 时还会返回 `grobid_available`、`grobid_status_code`、`grobid_url` 和 `grobid_error`。
